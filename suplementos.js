...
async function renderSuplementos(suplemento) {
    ...
    const $suplementos = await fetch(`http://localhost:5000/suplemento?titulo=${suplemento}`, requestOptions)
    .then(res => res.json())
    .then(res => {
        console.log(res);
...
document.getElementById('main-search').addEventListener('submit', function(event) {
    event.preventDefault(); // Evita el comportamiento predeterminado del formulario
    
    const searchInput = document.querySelector('.main-nav__search input[type="search"]').value;
